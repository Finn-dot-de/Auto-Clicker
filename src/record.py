...
from src.globals import ereignisse_queue, \
    stop_event  # Importiert globale Variablen und Events aus dem globals-Modul
...
def bei_tastendruck(taste):
    ...
    if aufzeichnung:  # Wenn die Aufzeichnung läuft
        try:
            ...
            ereignisse_queue.put(
                ('tastendruck', time.time(), ereignis_daten))  # Fügt das Ereignis zur Ereignis-Queue hinzu
        except AttributeError:
            ereignisse_queue.put(
                ('tastendruck', time.time(), str(taste)))  # Falls keine Zeichentaste, als String hinzufügen
...
def bei_tastenfreigabe(taste):
    ...
    if aufzeichnung:  # Wenn die Aufzeichnung läuft
        ...
        ereignisse_queue.put(
            ('tastenfreigabe', time.time(), ereignis_daten))  # Fügt das Ereignis zur Ereignis-Queue hinzu
...
def starte_tastatur_listener():
    with keyboard.Listener(on_press=bei_tastendruck,
                           on_release=bei_tastenfreigabe) as listener:  # Startet den Listener für Tastendrücke und -freigaben
        listener.join()  # Wartet, bis der Listener beendet wird
...
def starte_aufzeichnung():
    ...
    maus_thread = threading.Thread(target=starte_maus_listener,
                                   daemon=True)  # Erstellt einen Thread für den Maus-Listener
    tastatur_thread = threading.Thread(target=starte_tastatur_listener,
                                       daemon=True)  # Erstellt einen Thread für den Tastatur-Listener
...
def stoppe_aufzeichnung():
    ...
    dateipfad = filedialog.asksaveasfilename(defaultextension=".txt", filetypes=[
        ("Textdateien", "*.txt")])  # Öffnet einen Dateidialog zum Speichern der Ereignisse
    if dateipfad:
        try:
            ...
            messagebox.showinfo("Information",
                                f"Ereignisse gespeichert in {dateipfad}")  # Zeigt eine Nachricht an, dass die Ereignisse gespeichert wurden
